...
if uploaded_file:
    ...
    if df_lotto is not None and not df_lotto.empty:
                    ...
                    if not riga_tecnica_df.empty:
                        ...
                        if pd.notna(terzo_bari_val) and pd.notna(terzo_cagliari_val):
                            ...
                            try:
                                ...
                                st.markdown("---")
                                st.info("I numeri evidenziati nell'estrazione includono l'ambata calcolata, il 3° numero di Bari e Cagliari, più i loro antecedenti e successivi.")
                                
                            except (ValueError, TypeError) as e:
                                st.error(f"Errore conversione numeri: {e}")
                        else:
                            st.warning("Dati mancanti per il calcolo della tecnica in questa estrazione.")
                    else:
                        st.warning("Nessun dato trovato per la data selezionata.")
                except Exception as e:
                    st.error(f"Errore nel calcolo della tecnica: {e}")
        ...
        with col2:
            ...
            # Ottieni data corrente per la visualizzazione 
            data_corrente = date_uniche_dt_desc[st.session_state.visual_date_index_desc]  # Data dalla lista DESC
            
            # Pulsanti per navigare AVANTI/INDIETRO nel tempo usando la logica dell'indice
            nav_col1, nav_col2, nav_col3 = st.columns([1, 2, 1])
            
            # Pulsante PRECEDENTE (indietro nel tempo = avanti nella lista DESC)
            with nav_col1:
            ...
            # Pulsante SUCCESSIVA (avanti nel tempo = indietro nella lista DESC)
            with nav_col3:
            ...
            # Aggiorna UI per visualizzare l'estrazione corrispondente
            # Filtra il DataFrame per la data corrente 
            ...
            if not df_estrazione.empty:
                ...
                # Crea pivot per visualizzazione dei numeri delle ruote
                estratti_ruota = {}
                
                # Cerca le colonne per ogni ruota dinamicamente
                for ruota in ruote:
                    cols = [col for col in df_estrazione.columns if col.startswith(ruota)]
                    if len(cols) >= 5:  # Assicurati di avere abbastanza colonne
                        numeri = []
                        for i in range(5):  # Solo 5 numeri per ruota
                            try:
                                val = estrazione[cols[i]]
                                if pd.notna(val):
                ...
                # Costruisci un DataFrame per visaulizzazione e styling
                df_visual = pd.DataFrame(estratti_ruota, index=["1°", "2°", "3°", "4°", "5°"])
                
                # Applica lo styling condizionale per evidenziare i numeri della tecnica
                # e passa le ruote che vuoi controllare
                if numeri_da_evidenziare:
                    ruote_da_controllare = ["Bari", "Cagliari", "Firenze", "Genova", "Milano",
                                           "Napoli", "Palermo", "Roma", "Torino", "Venezia", "Nazionale"]
                    styled_df = df_visual.style.apply(
                        highlight_numbers, 
                        numeri_da_evidenziare=numeri_da_evidenziare,
                        ruote_da_evidenziare=ruote_da_controllare,
                        axis=None
                    )
                ...
                st.warning(f"Nessun dato trovato per la data {data_corrente.strftime('%d/%m/%Y')}")
                
    else:
        st.warning("Il file caricato non contiene dati validi o è vuoto.")
else:
    st.info("Carica un file CSV per iniziare l'analisi.")
    
    # Mostra istruzioni di utilizzao
    with st.expander("📚 Istruzioni di utilizzo"):
        ...
        1. **Carica un file CSV** contenente le estrazioni del Lotto.
           - Il file deve avere una struttura con la colonna "Data" e le colonne per i numeri estratti.
           - L'app salta le prime 3 righe del CSV (intestazioni o altre informazioni).
           
        2. **Tecnica Lotto**: Nella colonna di sinistra, seleziona una data per calcolare la previsione basata sui terzi estratti di Bari e Cagliari.
           - L'app calcolerà l'ambata sommando le decine dei due numeri.
           - Verranno mostrati anche i numeri antecedenti e successivi.
           
        3. **Navigazione Estrazioni**: Nella colonna di destra puoi navigare tra le estrazioni con i pulsanti o selezionare una data specifica.
           - I numeri collegati alla tecnica saranno evidenziati in rosso.
           
        Per ottenere i migliori risultati, assicurati che il file CSV abbia una struttura corretta con colonne che iniziano con "Bari", "Cagliari", ecc., seguite dai numeri estratti.
        """)
