...
for per,fd in firsts.items():
    ...
    dec_b, dec_c, amb = calcola_tecnica_lotto(tb,tc)[2]
    sub = df_lotto[(df_lotto['period']==per)&(df_lotto['Data']>fd)]
    
    # Funzione corretta per trovare occorrenze
    def occ(nums):
        occurrences = []
        for _, row in sub.iterrows():
            data_str = row['Data'].date().strftime('%d/%m/%Y')
            for wheel in ruote_da_evidenziare:
                # Prendi i 5 numeri della ruota corrente
                wheel_numbers = []
                for col in df_lotto.columns:
                    if col.startswith(wheel) and col != wheel:  # Esclude colonna con nome ruota
                        try:
                            wheel_numbers.append(int(row[col]))
                        except:
                            pass
                wheel_numbers = wheel_numbers[:5]  # Assicurati di prendere solo i primi 5
                
                # Controlla se tutti i numeri cercati sono presenti
                if all(n in wheel_numbers for n in nums):
                    occurrences.append(f"{wheel} {data_str}")
        
        return sorted(set(occurrences)) or ['-']
