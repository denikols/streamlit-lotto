...
def highlight_numbers(df):
    ...
    try:
        ...
        global numeri_da_evidenziare, numeri_adiacenti, ruote_da_evidenziare, numeri_ambo_trovato
        ...
        for idx, row in df.iterrows():
            ...
            for col in df.columns:
                ...
                if pd.notna(val) and isinstance(val, (int, float, np.number)):
                    try:
                        num_val = int(val)
                        # Verifica se è un numero dell'ambo trovato (priorità massima)
                        if num_val in numeri_ambo_trovato:
...
# --- NUOVA FUNZIONE: Verifica ambi nell'estrazione ---
def verifica_ambo(numeri_tecnica, dati_estrazione):
    """
    Verifica se almeno 2 dei 3 numeri della tecnica sono presenti nell'estrazione.
    ...
    Returns:
        (bool, list): Tupla con flag se trovato ambo e lista dei numeri trovati
    """
    # Converti tutti i numeri dell'estrazione in un set unico per la ricerca
    tutti_numeri_estratti = set()
    for ruota, numeri in dati_estrazione.items():
        for num in numeri:
            if pd.notna(num) and num is not None:
                tutti_numeri_estratti.add(int(num) if isinstance(num, (float, np.number)) else num)
    
    # Trova l'intersezione tra i numeri della tecnica e quelli estratti
    numeri_trovati = numeri_tecnica.intersection(tutti_numeri_estratti)
    
    # Se almeno 2 numeri coincidono, abbiamo un ambo
    return len(numeri_trovati) >= 2, list(numeri_trovati)
    ...
    if df_lotto is not None and not df_lotto.empty:
        ...
        with col2:
            ...
            if not estrazione_visual_df.empty:
                ...
                # --- VERIFICA AMBO: Se ci sono numeri di tecnica calcolati, verifica se sono presenti nell'estrazione ---
                ambo_trovato = False
                numeri_ambo = []
                
                if st.session_state.numeri_tecnica_calcolati:
                    ambo_trovato, numeri_ambo = verifica_ambo(st.session_state.numeri_tecnica_calcolati, dati_tabella)
