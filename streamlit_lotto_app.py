# Aggiungi questa funzione prima di tutto il codice esistente
def safe_highlight_numbers(df):
    try:
            ...
            for col in df.columns:
                ...
                if pd.notna(val) and isinstance(val, (int, float, np.number)):
                    try:
                        ...
                        if num_val in numeri_da_evidenziare and idx in ruote_da_evidenziare:
                            style_df.at[idx, col] = style_principale
                        # Verifica se è un numero adiacente (SOLO nelle ruote da evidenziare)
                        elif num_val in numeri_adiacenti and idx in ruote_da_evidenziare:
        ...
        # Se qualcosa va storto, non applicare stili
        st.error(f"Errore nella funzione highlight_numbers: {e}")
        return pd.DataFrame('', index=df.index, columns=df.columns)

# Sostituisci la funzione highlight_numbers originale con questa
highlight_numbers = safe_highlight_numbers
