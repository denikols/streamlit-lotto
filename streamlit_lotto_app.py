        ...
        if ss.mode in ['add_rec','edit_rec'] or ss.selected_recipe:
            ...
            if ss.mode in ['add_rec','edit_rec']:
                ...
                with st.form("form_rec"):
                    ...
                    # Mostra immagine corrente in dimensione ridotta
                    if rec and rec.get('image'):
                        st.image(rec['image'], caption="Foto attuale", width=200)
                ...
                # Visualizza dettagli ricetta selezionata con immagine ridotta
                rec = next(r for r in data['recipes'] if r['id'] == ss.selected_recipe)
                st.subheader(rec['title'])
                if rec.get('image'):
                    st.image(rec['image'], caption=rec['title'], width=200)
