...
def highlight_numbers(df):
    ...
    try:
        ...
        style_ambo = 'background-color: #FFD700; color: black; font-weight: bold;' # Nuovo stile per ambo (oro)
        
        # Accedi alle variabili globali per numeri principali e adiacenti
        global numeri_da_evidenziare, numeri_adiacenti, ruote_da_evidenziare, numeri_ambo_trovato, ruota_ambo_trovato
        ...
        for idx, row in df.iterrows():
            ...
            for col in df.columns:
                ...
                if pd.notna(val) and isinstance(val, (int, float, np.number)):
                    try:
                        num_val = int(val)
                        # Verifica se è un numero dell'ambo trovato E nella ruota specifica
                        if num_val in numeri_ambo_trovato and nome_ruota == ruota_ambo_trovato:
                            style_df.at[idx, col] = style_ambo
                        # Verifica se è un numero principale e in una ruota da evidenziare
                        elif num_val in numeri_da_evidenziare and nome_ruota in ruote_da_evidenziare:
...
# --- FUNZIONE MODIFICATA: Verifica ambi nell'estrazione ---
def verifica_ambo(numeri_tecnica, dati_estrazione):
    """
    Verifica se almeno 2 dei 3 numeri della tecnica sono presenti nella stessa ruota,
    ma limita il controllo solo alle ruote Bari, Cagliari, Nazionale.
    
    Args:
        numeri_tecnica: Set di numeri da verificare (ambata, terzo_bari, terzo_cagliari)
        dati_estrazione: Dizionario con le estrazioni (formato: {ruota: [n1, n2, n3, n4, n5]})
    
    Returns:
        (bool, list, str): Tupla con flag se trovato ambo, lista dei numeri trovati e nome della ruota
    """
    # Converti numeri_tecnica in un set per operazioni di intersezione
    numeri_tecnica_set = set(numeri_tecnica)
    
    # Ruote da controllare
    ruote_da_controllare = ["Bari", "Cagliari", "Nazionale"]
    
    # Variabili per memorizzare l'ambo migliore
    ambo_trovato = False
    numeri_ambo = []
    ruota_ambo = ""
    
    # Per ogni ruota selezionata, controlla se ci sono almeno 2 numeri della tecnica
    for ruota in ruote_da_controllare:
        # Verifica che la ruota esista nei dati dell'estrazione
        if ruota not in dati_estrazione:
            continue
            
        # Converti i numeri della ruota corrente in un set per l'intersezione
        numeri_ruota_set = set()
        for num in dati_estrazione[ruota]:
            if pd.notna(num) and num is not None:
                numeri_ruota_set.add(int(num) if isinstance(num, (float, np.number)) else num)
        
        # Trova l'intersezione tra i numeri della tecnica e quelli della ruota corrente
        matching_numbers = numeri_tecnica_set.intersection(numeri_ruota_set)
        
        # Se abbiamo almeno 2 numeri coincidenti nella stessa ruota, abbiamo un ambo
        if len(matching_numbers) >= 2:
            ambo_trovato = True
            numeri_ambo = list(matching_numbers)
            ruota_ambo = ruota
            break  # Ferma al primo ambo trovato
    
    return ambo_trovato, numeri_ambo, ruota_ambo
...
numeri_ambo_trovato = set()    # Numeri che compongono l'ambo trovato (oro)
ruota_ambo_trovato = ""        # Ruota in cui è stato trovato l'ambo
...
if uploaded_file:
    ...
    if df_lotto is not None and not df_lotto.empty:
        ...
        # Inizializza lo stato per i numeri di tecnica calcolati
        if 'numeri_tecnica_calcolati' not in st.session_state:
            st.session_state.numeri_tecnica_calcolati = set()
            st.session_state.data_tecnica_selezionata = None
            st.session_state.terzo_bari = None
            st.session_state.terzo_cagliari = None 
            st.session_state.ambata = None
                    ...
                    if not riga_tecnica_df.empty:
                        ...
                        if pd.notna(terzo_bari_val) and pd.notna(terzo_cagliari_val):
                            ...
                            try:
                                ...
                                # Salva i numeri nello stato della sessione per la verifica ambo
                                st.session_state.numeri_tecnica_calcolati = set([ambata, terzo_bari, terzo_cagliari])
                                st.session_state.data_tecnica_selezionata = data_scelta_tecnica
                                st.session_state.terzo_bari = terzo_bari
                                st.session_state.terzo_cagliari = terzo_cagliari
                                st.session_state.ambata = ambata
                                ...
                                # Aggiungi legenda per i colori e lista esplicita delle ruote monitorate
                                st.markdown("---")
                                ...
                                st.markdown("🟡 **Oro:** Numeri ambo trovato (se presenti)")
                                
                                st.info("**Ruote consigliate e monitorate per ambo:** Bari, Cagliari, Nazionale.")
        ...
        with col2:
            ...
            # Reset del set dei numeri ambo trovato all'inizio di ogni visualizzazione
            numeri_ambo_trovato.clear()
            ruota_ambo_trovato = ""
            ...
            if not estrazione_visual_df.empty:
                ...
                # --- VERIFICA AMBO: Se ci sono numeri di tecnica calcolati, verifica se sono presenti nelle ruote selezionate ---
                ambo_trovato = False
                numeri_ambo = []
                
                if st.session_state.numeri_tecnica_calcolati:
                    ambo_trovato, numeri_ambo, ruota_ambo = verifica_ambo(st.session_state.numeri_tecnica_calcolati, dati_tabella)
                    
                    if ambo_trovato:
                        # Aggiorna il set globale dei numeri ambo trovati per lo styling
                        numeri_ambo_trovato.update(numeri_ambo)
                        ruota_ambo_trovato = ruota_ambo
                        
                        # Crea un messaggio ben visibile per l'ambo trovato
                        st.success(f"""
                        ### 🎯 AMBO TROVATO! 🎯
                        **Data tecnica:** {st.session_state.data_tecnica_selezionata.strftime('%d/%m/%Y')}  
                        **Data estrazione:** {data_effettiva_visualizzazione.strftime('%d/%m/%Y')}
                        
                        **Numeri tecnica:** {st.session_state.terzo_bari} (Bari), {st.session_state.terzo_cagliari} (Cagliari), {st.session_state.ambata} (Ambata)  
                        **Numeri ambo trovati:** {', '.join(map(str, sorted(numeri_ambo)))}
                        **Ruota:** {ruota_ambo}
                        """)
                ...
                try:
                    ...
                    if numeri_da_evidenziare or numeri_ambo_trovato:
