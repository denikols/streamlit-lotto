...
for wheel in ruote_da_evidenziare:
    wheel_cols[wheel] = [col for col in df_lotto.columns if col.startswith(wheel) and col != wheel]
...
for per,fd in firsts.items():
    ...
    def occ(nums):
        ...
        for _, row in sub.iterrows():
            ...
            for wheel in ruote_da_evidenziare:
                # Ottieni le colonne numeriche per questa ruota (escludendo il nome della ruota)
                wheel_cols = [col for col in df_lotto.columns if col.startswith(f"{wheel} ")]
                
                # Se non ci sono colonne con questo formato, prova un altro formato
                if not wheel_cols:
                    wheel_cols = [col for col in df_lotto.columns if col.startswith(wheel) and col != wheel]
