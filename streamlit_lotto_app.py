...
def load_data(uploaded_file_content):
    ...
    try:
        ...
        # Identifica colonne Bari/Cagliari (assumendo posizioni 3 e 8)
        col_bari_3_idx = 3
        col_cagliari_3_idx = 8

        # Verifica indici colonne
        if col_bari_3_idx >= len(df.columns) or col_cagliari_3_idx >= len(df.columns):
             raise IndexError(f"Indici di colonna per Bari/Cagliari ({col_bari_3_idx}, {col_cagliari_3_idx}) fuori dai limiti. Verifica struttura CSV.")
...
def highlight_numbers(val, numeri_da_evidenziare):
    ...
    # Controlla se val è un numero valido e se è nel set dei numeri da evidenziare
    if isinstance(val, (int, float, np.number)) and pd.notna(val):
        try:
            # Confronta come intero
            if int(val) in numeri_da_evidenziare:
                return style
        except (ValueError, TypeError):
            pass # Ignora se non convertibile a int
    return '' # Nessuno stile altrimenti
    ...
    if df_lotto is not None and not df_lotto.empty:
        ...
        with col2:
            ...
            if btn_cols[0].button("⬅️ Estrazione Prec.", use_container_width=True, disabled=(idx_corrente_desc >= max_idx_desc), key="btn_prev"):
                ...
                # Non serve più st.rerun() qui, Streamlit lo gestisce automaticamente
                # aggiornando l'indice e rieseguendo lo script dal selectbox in poi.
            ...
            if not estrazione_visual_df.empty:
                ...
                if numeri_da_evidenziare:
                    try:
                         # Crea lo Styler e applica la funzione di highlight alle celle specificate
                         df_styler = df_visual_transposed.style.apply(
                              ...
                              subset=pd.IndexSlice[ruote_da_evidenziare, :] # Solo righe Bari, Cagliari, Nazionale
