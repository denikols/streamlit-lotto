...
for per,fd in firsts.items():
    ...
    amb = calcola_tecnica_lotto(tb,tc)[2]
    sub = df_lotto[(df_lotto['period']==per)&(df_lotto['Data']>fd)]
    def occ(nums):
        return sorted({f"{w} {r['Data'].date().strftime('%d/%m/%Y')}" for _,r in sub.iterrows() for w in ruote_da_evidenziare if all(n in [r[c] for c in df_lotto.columns if c.startswith(w)][:5] for n in nums)}) or ['-']
    records.append({'Mese':per.strftime('%Y-%m'),'Ambi':'; '.join(occ([tb,tc])),'Ambetti':'; '.join(occ([amb,tb])+occ([amb,tc])),'Terni':'; '.join(occ([amb,tb,tc]))})
